...
- **[publish-a-skill](./skills/productivity/publish-a-skill/SKILL.md)** — Wire a newly-authored skill into `plugin.json`, the top-level README, and the bucket README, then commit and push. Companion to `write-a-skill`.
- **[write-a-skill](./skills/productivity/write-a-skill/SKILL.md)** — Create new skills *or* improve existing ones, with a v2 eval/A-B loop for measuring impact. Vendored from Anthropic's `skill-creator`.
